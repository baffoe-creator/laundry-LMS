...
def set_exe_build_timestamp(exe_path, timestamp):
    ...
    # Workaround for pefile double-escape bug on Windows (PyInstaller #8870):
    # os.stat() inside pefile chokes on double-escaped backslashes, so
    # convert to a plain forward-slash string which Windows accepts fine.
    exe_path = str(exe_path).replace('\\\\', '\\').replace('\\', '/')
